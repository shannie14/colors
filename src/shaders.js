/**
 * colors — GLSL Shaders
 * Navier-Stokes fluid simulation + rendering pipeline.
 *
 * Algorithm overview:
 *   1. Advection — move velocity/dye fields along the velocity field
 *   2. Diffusion — viscous spreading (implicit via Jacobi iteration)
 *   3. Force application — user interaction injects velocity + color
 *   4. Pressure solve — Jacobi iteration to enforce incompressibility
 *   5. Projection — subtract pressure gradient to get divergence-free velocity
 *   6. Rendering — map dye field to screen with optional bloom post-processing
 */
...
export const bloomPrefilterShader = `
  ...
  void main() {
    ...
    float soft = brightness - threshold + curve.y;
    soft = clamp(soft, 0.0, curve.z);
    soft = soft * soft * curve.x;
...
export const bloomFinalShader = `
  ...
  void main() {
    gl_FragColor = intensity * texture2D(uTexture, vUv);
  }
`;
